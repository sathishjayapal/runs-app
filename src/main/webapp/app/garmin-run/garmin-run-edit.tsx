...
export default function GarminRunEdit() {
  const { t } = useTranslation();
  useDocumentTitle(t('garminRun.edit.headline'));
  ...
  const prepareForm = async () => {
    try {
      ...
      const data = (await axios.get('/api/garminRuns/' + currentId)).data;
  ...
  const updateGarminRun = async (data: GarminRunDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.put('/api/garminRuns/' + currentId, data);
      navigate('/garminRuns', {
            state: {
              msgSuccess: t('garminRun.update.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('garminRun.edit.headline')}</h1>
      <div>
        <Link to="/garminRuns" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('garminRun.edit.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(updateGarminRun)} noValidate>
