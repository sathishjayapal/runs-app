...
export default function JournalEntryAdd() {
    ...
    const createJournalEntry = async (data: JournalEntryDTO) => {
        ...
        try {
            ...
            navigate('/journalEntries', {
                state: {
                    msgSuccess: t('journalEntry.create.success')
                }
            });
        } catch (error: any) {
    ...
    return (<>
        <div className="flex flex-wrap mb-6">
            <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('journalEntry.add.headline')}</h1>
            <div>
                <Link to="/journalEntries"
                      className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('journalEntry.add.back')}</Link>
            </div>
        </div>
        <form onSubmit={useFormResult.handleSubmit(createJournalEntry)} noValidate>
            ...
            <input type="submit" value={t('journalEntry.add.headline')}
